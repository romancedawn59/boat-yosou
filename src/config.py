...
# 本命の選別対象場。2026-09-21ケンさん決定で「現行5場」の絞りを廃止し全24場から選ぶ
# (5場・帯・低い順の補強の重ね掛けが検証結果に誤解を与えた可能性)。
# TARGET_VENUE_CODESは場別ページ・気象・要オッズ確認(30〜35%帯)の表示用に残す。
HONMEI_VENUE_CODES = list(range(1, 25))

# === v2(ケンさん案・2026-07-18ユーザー決定/検証はtest/verify_ken_v2*.py) ===
# 予測は全24場で行い、購入対象は
#   本命   = 全24場(HONMEI_VENUE_CODES)×1位勝率22.5〜30%のうち1位勝率が低い順に最大HONMEI_CAPレース
...
HONMEI_PROB_MAX = 0.30  # 本命の判定閾値(2026-07-20ユーザー決定で35%→30%。28〜35%帯は
                        # 利益貢献ゼロの詰め物で、30%は利益維持のまま投資-25%・DD-30%。
                        # 検証: test/verify_honmei_threshold.py。表示上の「荒れ注意」
                        # ラベル(bucket_ofの0.35)は据え置き。30〜35%帯は要注目で前向き観測)
HONMEI_CAP = 4          # 本命(20-30%帯)の1日上限(2026-08-04検証⑰: cap4×1,400円採用)
DAILY_BUDGET = 10200    # 日次予算上限。超混戦は超過許可・本命は残予算内(ケンさん設計)
KONSEN_UNIT = 2000      # 超混戦1レースの予算(⑬構成)
HONMEI_UNIT = 1400      # 本命の選別用予算単位(検証済み選別ルールを維持。実際の構成はH静的スリム1,000円)
