...
from config import JST, PROJECT_DIR, jst_today
...
def run_predict():
    import predict
    # クラウドランナーはUTCのため「今日」は必ずJSTで判定する
    predict.run(jst_today())


def run_exhibition():
    import collect_exhibition
    # races.deadline_timeはnaive(JST)の文字列のため、JSTの現在時刻をnaiveにして渡す
    now = datetime.now(JST).replace(tzinfo=None)
    collect_exhibition.collect_today(now.date(), now)
