...
def ken_portfolio(
    ...
    - konsen=True(超混戦帯)は案1「拾える複厚」(2026-07-29判断会採用)。
      Q案(2026-07-21採用の7点)をスロット分解した結果、C複r1r3r4(95.9%)と
      D軸外しr2r3r4(97.6%)が単体赤字で、当たっても投資割れ(ガミ)の主因だった。
      その200円を当たり頭の複2点(A複149%・B複226%)に乗せ替える。
      検証: test/verify_konsen_slots_q.py(回収率191.4%→209.6%、ガミ33.6%→13.1%、
      月次8か月中7か月で改善・最悪月も黒字化)。代償は完全外れ率+18pt。
      E/F単(差され単・市場がフェア比2.48倍で安売り)は不可侵。
      深い波乱(r3=r4=r5)は全滅保険として残す(verify_deep_upset.py)。
      ※変形ハ(A/Bトリオ確率1位の単2点・202.1%)は紙上並走中。9/1に昇格判定
      (test/verify_konsen_ab_top_orders.py。picksのrankedから遡及再構成可能)
    """
    ...
    if confidence == "荒れ注意" and konsen and r5 is not None:
        # 案1「拾える複厚」×2(2026-07-31ケンさん決定: 超混戦は検証済みエッジのため
        # 2,000円に増資。ハードストップは-8万→-10万に改定)。
        # 配分の傾斜は維持(均等化はフォーメーション入力が楽になるが-4.0ptの検証結果)
        return [
            ("3連複", trio(r1, r2, r3), 600, "検証済み"),
            ("3連複", trio(r1, r2, r4), 400, "検証済み"),
            ("3連単", f"{r3}-{r1}-{r2}", 400, "検証済み"),
            ("3連単", f"{r4}-{r1}-{r2}", 400, "検証済み"),
            ("3連複", trio(r3, r4, r5), 200, "深い波乱"),
        ]
